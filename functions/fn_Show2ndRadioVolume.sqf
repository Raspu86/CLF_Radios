/*
 	Name: CLF_fnc_Show2ndRadioVolume
...
if !(_isDDRadio) then
{
	...
	_stereo = localize format ["STR_additional_stereo_settings_%1", if(_isLrRadio) then {_radio call TFAR_fnc_getAdditionalLrStereo} else {_radio call TFAR_fnc_getSwStereo}];
